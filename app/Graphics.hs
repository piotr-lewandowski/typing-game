...
renderSnippet (SnippetInProgress typ left progress) ok =
    pictures
...
renderScore config game =
    translate (-fromIntegral (config ^. width) / 2) (fromIntegral (config ^. height) / 2 - 40) $
        color green $
            scale 0.3 0.3 $
                text $
...
renderLifes config game imgs =
    translate (fromIntegral (config ^. width) / 2 - 2 * totalW) (fromIntegral (config ^. height) / 2 - w / 2 - 5) $
        pictures
...
renderTimeLeft config game =
    translate (fromIntegral (config ^. width) / 2 - 15) 0 $
        color red $
...
renderLevelSelect config (LevelSelectState lvls ind) title = pictures $ placedTitle : zipWith renderLevel [0 ..] lvls
  where
    ...
    renderLevel i (Level num _ _) =
        translate (50 - fromIntegral (config ^. width) / 2) (100 + fromIntegral (-i) * 50) $
